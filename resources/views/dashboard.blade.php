        ...
        <form method="GET" action="{{ route('produtos.search') }}" class="form-inline">
            <div class="br-form-group mb-2">
                ...
                <input type="text" name="query" id="query"
                    class="form-control form-control-sm form-control-custom mr-2" placeholder="Nome ou Descrição"
                    value="{{ request('query') }}">
            </div>
                        ...
                        <tr>
                            ...
                            <td>
                                <form action="{{ route('produtos.destroy', $produto->id) }}" method="post">
                                    @csrf
                                    @method('DELETE')
                                    <a class="br-button circle secondary m-2"
                                        href="{{ route('produtos.show', $produto->id) }}"><i
                                            class="fas fa-eye"></i></a>
                                    <a class="br-button circle secondary m-2"
                                        href="{{ route('produtos.edit', $produto->id) }}"><i
                                            class="fas fa-edit"></i></a>
                                    <button type="submit" class="br-button circle secondary m-2"><i
                                            class="fas fa-spin fa-trash"></i></button>
                                </form>
                            </td>
                        </tr>
                    @endforeach
